...
io.on('connection', function (socket) {
    ...
    var initialize_member = get_members.then(function (members) {
        ...
        // redis.zadd('members_rank', member.lastTime, JSON.stringify(member)).then(
        //     function () {
        //         redis.zrange('members_rank', 0, 0).then(function (x) {
        //             var first = JSON.parse(x);
        //             console.log(first);
        //             if (first.socket != member.socket) {
        //                 console.log("second");
        //                  for (button in first.buttons) {
        //                      if (!button.valid) {
        //                         console.log(button.valid);
        //                     //     console.log("hah");
        //                     //     redis.zrem('members_rank', JSON.stringify(first_member)).then(
        //                     //         function(){
        //                     //             first_member.lastTime = moment.now();
        //                     //             redis.zadd('members_rank', first_member.lastTime, JSON.stringify(first_member));
        //                     //         }
        //                     //     );
        //                     //     redis.hdel('members', first_member.socket).then(
        //                     //         function(){
        //                     //             button.socket_id = member.socket;
        //                     //             button.valid = true;
        //                     //             redis.hset('members', first_member.socket, JSON.stringify(first_member));
        //                     //         }
        //                     //     );
    ...
    Promise.all([initialize_member, get_members, get_messages]).then(function (values) {
        ...
        socket.on('disconnect', function () {
            ...
            //redis.zrem('members_rank', JSON.stringify(members[socket.id]));
